...
import { Search, Play, Loader, Film, Tv, CheckCircle, AlertCircle, Globe, List, Download, Copy, ExternalLink, X, Maximize } from 'lucide-react';
...
export default function App() {
	...
	// États pour le player
	const [streamUrl, setStreamUrl] = useState(null);
	const [isStreamReady, setIsStreamReady] = useState(false); // Nouvel état pour l'attente
	const [torrentResults, setTorrentResults] = useState([]);
	const [activeTorrent, setActiveTorrent] = useState(null);
	const [error, setError] = useState(null);
	const videoRef = useRef(null);
	const pollingRef = useRef(null); // Pour stocker l'intervalle de vérification
	...
	// Fonction pour attendre que le backend soit prêt (Polling)
	const waitForStream = (hostname) => {
		setIsStreamReady(false);

		// On nettoie l'ancien intervalle s'il existe
		if (pollingRef.current) clearInterval(pollingRef.current);

		pollingRef.current = setInterval(async () => {
			try {
				// On interroge le endpoint /meta du streamer
				...
				if (data.ready) {
					// C'est prêt ! On arrête de vérifier et on lance la vidéo
	...
	// Nettoyage de l'intervalle quand on quitte
	useEffect(() => {
		return () => {
			if (pollingRef.current) clearInterval(pollingRef.current);
		};
	...
	const handleStream = async (movie, lang) => {
		...
		try {
			...
			if (data && data.length > 0) {
				...
				// On lance l'attente active au lieu d'afficher direct
				const hostname = window.location.hostname;
				waitForStream(hostname);

				// On met une URL temporaire juste pour afficher l'overlay (mais pas le player)
	...
	const forceDownload = async (torrent) => {
		...
		setStreamUrl("loading"); // Mode chargement
		setIsStreamReady(false);
		...
		try {
			...
			setError("Erreur changement torrent");
		}
	};

	return (
		<div className="min-h-screen bg-gray-900 text-white font-sans">
			{/* --- OVERLAY PLAYER CINÉMA --- */}
				...
				<div className="fixed inset-0 z-[100] bg-black/95 flex flex-col items-center justify-center animate-in fade-in duration-300">
					...
					<div className="absolute top-0 w-full p-4 flex justify-between items-center bg-gradient-to-b from-black to-transparent z-50">
						<div className="flex items-center gap-4">
							<button onClick={() => { setStreamUrl(null); setIsStreamReady(false); if(pollingRef.current) clearInterval(pollingRef.current); }} className="p-2 hover:bg-white/20 rounded-full transition">
								<X size={32} />
							</button>
							<div>
								<h2 className="font-bold text-xl">{cleanTitle(activeTorrent.title)}</h2>
								{isStreamReady ? (
									<span className="text-sm text-green-400 flex items-center gap-1"><CheckCircle size={14}/> Transcodage Live Actif</span>
								) : (
									<span className="text-sm text-yellow-400 flex items-center gap-1"><Loader className="animate-spin" size={14}/> Démarrage du moteur...</span>
								)}
							</div>
						</div>
					</div>

					{/* LE LECTEUR VIDÉO OU LOADER */}
					<div className="w-full max-w-6xl aspect-video bg-black shadow-2xl relative group flex items-center justify-center">
						{!isStreamReady ? (
							<div className="text-center space-y-4">
								...
								<p className="text-xl font-bold">Préparation du film...</p>
								<p className="text-gray-400">Analyse du fichier MKV en cours.</p>
							</div>
						) : (
							<video
								ref={videoRef}
								...
								controls
								autoPlay
								// IMPORTANT : On ajoute un timestamp pour éviter le cache du navigateur
					...
					<div className="absolute bottom-10 w-full max-w-4xl px-4 z-50">
						{torrentResults.length > 1 && (
							<div className="bg-gray-900/80 backdrop-blur rounded-xl p-4 border border-gray-700 max-h-40 overflow-y-auto">
								<p className="text-xs text-gray-400 mb-2 uppercase font-bold tracking-wider">Autres versions disponibles</p>
								{torrentResults.map((t, idx) => (
									<div key={idx}
										...
										{t.href === activeTorrent.href && <span className="text-xs border border-red-500 px-2 rounded">Lecture</span>}
									</div>
								))}
							</div>
						)}
			...
			{/* Reste de l'interface (Navbar, Hero, Grid) inchangé */}
